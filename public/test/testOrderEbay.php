...
$request .= '<eBayAuthToken>v^1.1#i^1#r^1#p^3#I^3#f^0#t^Ul4xMF8xOjk3MDVGODY4MkI3QUI4QkZGNzlGRTAwMjQwMjk4NkI4XzBfMSNFXjI2MA==</eBayAuthToken>
  ...
  <CreateTimeFrom>' . $create . '</CreateTimeFrom>
  <CreateTimeTo>' . $now . '</CreateTimeTo>
...
foreach ($ordersArray as $orders) {
    ...
    foreach ($orders as $order) {
        $Name = '';
        $surname = '';
        $marketplaceOrderId = preg_replace('/[\{\}]/','',$order->ShippingDetails->SellingManagerSalesRecordNumber);
        ...
        $nameCustomer = null;
        foreach ($order->ShippingAddress as $val) {
            $Name = preg_replace('/[\{\}]/','',$val->Name);
            $surname = "da Ebay";


            $street1 = preg_replace('/[\{\}]/','',$val->Street1);
            $street2 = preg_replace('/[\{\}]/','',$val->Street2);
            $cityName = preg_replace('/[\{\}]/','',$val->CityName);
            $StateOrProvince = preg_replace('/[\{\}]/','',$val->StateOrProvince);
            $Country = preg_replace('/[\{\}]/','',$val->Country);
            $CountryName = preg_replace('/[\{\}]/','',$val->CountryName);
            $Phone = preg_replace('/[\{\}]/','',$val->Phone);
            $PostalCode = preg_replace('/[\{\}]/','',$val->PostalCode);
            $AddressId = preg_replace('/[\{\}]/','',$val->AddressId);
            $AddressOwner = preg_replace('/[\{\}]/','',$val->AddressOwner);
        }
        $EmailEbay = $order->TransactionArray->Transaction->Buyer->Email;
        echo $EmailEbay.'<br>';
        $orderIDMarketplace = preg_replace('/[\{\}]/','',$order->OrderID);
